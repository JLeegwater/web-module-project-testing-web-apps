...
test("renders all firstName, lastName and email text when submitted. Does NOT render message if message is not submitted.", async () => {
  ...
  userEvent.type(email, "JLeegwater@live.com");

  //   const msg = screen.getByLabelText(/Message/i);
  //   userEvent.type(msg, "BOIIIIII");

  userEvent.click(screen.getByRole("button"));

  const testFirst = screen.getByTestId("firstnameDisplay");
  expect(testFirst).toHaveTextContent("First Name: Jesse");

  const testLast = screen.getByTestId("lastnameDisplay");
  expect(testLast).toHaveTextContent("Last Name: Leegwater");

  const testEmail = screen.getByTestId("emailDisplay");
  expect(testEmail).toHaveTextContent("Email: JLeegwater@live.com");

  expect(screen.queryByTestId("messageDisplay")).not.toBeInTheDocument;

  const message = screen.getByLabelText(/message/i);
  userEvent.type(message, "This is for Jesse Leegwater");

  const testMessage = screen.getByTestId("messageDisplay");
  expect(testMessage).toHaveTextContent("This is for Jesse Leegwater");
});
